...
(() => {
  ...
  /* -------------------------------------------------------
     Email Validation → Enable/Disable Request OTP button
     ------------------------------------------------------- */
  function updateRequestOtpButton() {
    const emailValid = fields.email.validate(fields.email.input.value);
    // Only enable if email is valid AND not currently loading AND not already verified
    requestOtpBtn.disabled = !emailValid || requestOtpBtn.classList.contains('is-loading') || otpVerified;
  }
  ...
  Object.keys(fields).forEach(name => {
    ...
    input.addEventListener('input', () => {
      ...
      // Update Request OTP button when email changes
      if (name === 'email') {
  ...
  /* -------------------------------------------------------
     Restrict Phone to numeric characters only
     ------------------------------------------------------- */
  ...
  function startResendTimer() {
    ...
    otpResend.innerHTML = `
      ...
      <span id="resendText">Resend OTP in <strong id="resendTimer">${formatTime(countdown)}</strong></span>
    `;
